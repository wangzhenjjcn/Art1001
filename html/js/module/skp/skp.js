...
function begin_loading(target){
    var now=$(target).index();
    if($(target).children(".download").length==0){
        $(target).prepend("<img src='images/download.gif' class='download' style='display:block'/>");
    }  
    $(target).children(".download").css({"display":"block"});  
    $(target).css({
        "position":"relative",
    });
    $(".download").css({
